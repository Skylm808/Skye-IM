# User 微服务架构与流程讲解

## 一、服务概述

User 服务是 SkyeIM 即时通讯系统的**用户管理微服务**，负责用户资料的查询和更新，以及用户搜索功能。

**注意**: 用户的注册、登录、鉴权等功能在 **Auth 服务**中实现，User 服务专注于用户资料管理。

### 技术栈

| 组件 | 技术选型 | 用途 |
|------|---------|------|
| 框架 | go-zero | 微服务框架 |
| 数据库 | MySQL | 用户数据持久化 |
| 缓存 | Redis | Model 缓存 |
| 通信 | gRPC + HTTP | RPC 内部调用 + HTTP 对外接口 |

---

## 二、目录结构

```
app/user/
├── api/                          # HTTP API 层
│   ├── user.api                  # API 定义文件
│   ├── USER_API文档.md          # 前端对接文档
│   └── internal/
│       ├── handler/              # HTTP 处理器
│       │   ├── profile/          # 资料管理
│       │   └── search/           # 搜索功能
│       └── logic/                # 业务逻辑层
│           ├── profile/
│           └── search/
└── rpc/                          # gRPC 服务层
    └── internal/
        └── logic/                # RPC 业务逻辑
```

**注意**: User 表定义在 `app/auth/model/user.sql`

---

## 三、数据库设计

### 3.1 用户表 (user)

```sql
CREATE TABLE `user` (
    `id` bigint unsigned NOT NULL AUTO_INCREMENT,
    `username` varchar(64) NOT NULL COMMENT '用户名（唯一）',
    `phone` varchar(20) DEFAULT NULL COMMENT '手机号（唯一）',
    `email` varchar(128) DEFAULT NULL COMMENT '邮箱（唯一）',
    `password` varchar(128) NOT NULL COMMENT '密码（bcrypt加密）',
    `nickname` varchar(64) DEFAULT '' COMMENT '昵称',
    `avatar` varchar(512) DEFAULT '' COMMENT '头像URL',
    `signature` varchar(255) DEFAULT '' COMMENT '个性签名',
    `gender` tinyint unsigned NOT NULL DEFAULT 0 COMMENT '性别：0-未知 1-男 2-女',
    `region` varchar(64) DEFAULT '' COMMENT '地区',
    `status` tinyint unsigned NOT NULL DEFAULT 1 COMMENT '状态：1-正常 0-禁用',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_username` (`username`),
    UNIQUE KEY `uk_phone` (`phone`),
    UNIQUE KEY `uk_email` (`email`),
    KEY `idx_status` (`status`),
    KEY `idx_created_at` (`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```

**设计要点**：

**1. 唯一约束**:
- `uk_username`: 用户名全局唯一
- `uk_phone`: 手机号全局唯一
- `uk_email`: 邮箱全局唯一

**用途**: 防止重复注册，支持多种登录方式

**2. 密码字段**:
- 使用 bcrypt 加密存储
- 不可逆，安全性高

**3. 资料字段**:
| 字段 | 类型 | 说明 |
|------|------|------|
| nickname | varchar(64) | 昵称（可重复）|
| avatar | varchar(512) | 头像URL |
| signature | varchar(255) | 个性签名 |
| gender | tinyint | 0-未知 1-男 2-女 |
| region | varchar(64) | 地区 |

**4. 状态字段**:
- `status=1`: 正常
- `status=0`: 禁用（封号）

**5. 索引优化**:
| 索引 | 用途 |
|------|------|
| `uk_username` | 登录、搜索 |
| `uk_phone` | 手机号登录、搜索 |
| `uk_email` | 邮箱登录、搜索 |
| `idx_status` | 过滤禁用用户 |

---

## 四、核心流程

### 4.1 获取用户资料流程

```
┌─────────┐    GET /api/v1/user/profile    ┌─────────────┐
│  前端   │ ───────────────────────────────► │  Handler    │
└─────────┘  Authorization: Bearer token   └──────┬──────┘
                                                   │
                                                   ▼
                                            ┌─────────────┐
                                            │   Logic     │
                                            │ GetProfile  │
                                            └──────┬──────┘
                                                   │
                                                   │ JWT中提取userId
                                                   ▼
                                            ┌─────────────┐
                                            │ User RPC    │
                                            │ GetUserInfo │
                                            └──────┬──────┘
                                                   │
                                                   ▼
                                            ┌─────────────┐
                                            │ User Model  │
                                            │ (带缓存)    │
                                            └──────┬──────┘
                                                   │
                    ┌──────────────────────────────┼──────────────┐
                    │                              │              │
                    ▼                              ▼              ▼
             ┌─────────────┐              ┌─────────────┐ ┌─────────────┐
             │ 查 Redis    │              │ 未命中      │ │ 查 MySQL    │
             │ 缓存        │              │             │ │             │
             └──────┬──────┘              └─────────────┘ └──────┬──────┘
                    │                                            │
                    │ 命中                                        │
                    ▼                                            ▼
             ┌─────────────┐                              ┌─────────────┐
             │ 返回用户信息│                              │ 写入缓存    │
             └─────────────┘                              └──────┬──────┘
                                                                 │
                                                                 ▼
                                                          ┌─────────────┐
                                                          │ 返回用户信息│
                                                          └─────────────┘
```

**关键点**：
- 🔐 userId 从 JWT Token 中提取
- 🚀 Model 层自动缓存，提升性能
- 📝 返回完整用户信息（包括手机号、邮箱）

---

### 4.2 更新用户资料流程

```
┌─────────┐    PUT /api/v1/user/profile    ┌─────────────┐
│  前端   │ ───────────────────────────────► │  Handler    │
└─────────┘  {nickname, signature, ...}    └──────┬──────┘
                                                   │
                                                   ▼
                                            ┌─────────────┐
                                            │   Logic     │
                                            │UpdateProfile│
                                            └──────┬──────┘
                                                   │
                ┌──────────────────────────────────┼──────────────┐
                │                                  │              │
                ▼                                  ▼              ▼
         ┌─────────────┐              ┌─────────────┐ ┌─────────────┐
         │ 参数验证    │              │ 手机号检查  │ │ 邮箱检查    │
         │             │              │ (唯一性)    │ │ (唯一性)    │
         └─────────────┘              └──────┬──────┘ └──────┬──────┘
                                             │              │
                                             │              │
                                             ▼              ▼
                                      ┌─────────────────────────┐
                                      │ 查询数据库检查重复      │
                                      │ WHERE phone = ? AND id != ? │
                                      └──────┬──────────────────┘
                                             │
                                             │ 不重复
                                             ▼
                                      ┌─────────────┐
                                      │ 更新数据库  │
                                      │ UPDATE user │
                                      └──────┬──────┘
                                             │
                                             ▼
                                      ┌─────────────┐
                                      │ 清除缓存    │
                                      └──────┬──────┘
                                             │
                                             ▼
                                      ┌─────────────┐
                                      │ 返回新资料  │
                                      └─────────────┘
```

**关键点**：
- ✅ **唯一性检查**：手机号和邮箱需要检查是否被其他用户使用
- 🔒 **更新后清缓存**：保证缓存一致性
- 📝 **返回最新数据**：更新成功后返回完整用户信息

---

### 4.3 搜索用户流程

**精确搜索（用于添加好友）**:
```
用户输入完整用户名/手机号/邮箱
    ↓
GET /api/v1/user/search?keyword=skylm808
    ↓
查询数据库
WHERE username = 'skylm808' 
   OR phone = '13800138000' 
   OR email = 'skylm@example.com'
    ↓
找到用户
    ↓
脱敏处理
phone: 138****8000
email: sky****@example.com
    ↓
返回结果
```

**全局模糊搜索（搜索栏）**:
```
用户输入关键词
    ↓
GET /api/v1/user/search/global?keyword=技术
    ↓
查询用户
WHERE nickname LIKE '%技术%'
    ↓
查询群组（调用 Group RPC）
WHERE name LIKE '%技术%'
    ↓
合并结果
    ↓
返回 {users: [...], groups: [...]}
```

---

## 五、核心组件详解

### 5.1 隐私保护机制

**场景一：获取自己的资料**
```go
func GetOwnProfile(userId int64) (*UserInfo, error) {
    user := getUserById(userId)
    // 返回完整信息
    return user, nil
}
```

**场景二：搜索其他用户**
```go
func SearchUser(keyword string) ([]*UserInfo, error) {
    users := searchByKeyword(keyword)
    
    // 脱敏处理
    for _, user := range users {
        user.Phone = maskPhone(user.Phone)  // 138****8000
        user.Email = maskEmail(user.Email)  // sky****@example.com
    }
    
    return users, nil
}

func maskPhone(phone string) string {
    if len(phone) != 11 {
        return phone
    }
    return phone[:3] + "****" + phone[7:]
}

func maskEmail(email string) string {
    parts := strings.Split(email, "@")
    if len(parts) != 2 {
        return email
    }
    
    prefix := parts[0]
    if len(prefix) > 3 {
        prefix = prefix[:3] + "****"
    }
    
    return prefix + "@" + parts[1]
}
```

**场景三：查看其他用户资料**
```go
func GetUserInfo(userId int64, targetUserId int64) (*UserInfo, error) {
    user := getUserById(targetUserId)
    
    // 不返回隐私字段
    user.Phone = ""
    user.Email = ""
    
    return user, nil
}
```

---

### 5.2 Model 缓存机制

**go-zero 自动缓存**:
```go
// 查询时自动缓存
func (m *defaultUserModel) FindOne(ctx context.Context, id int64) (*User, error) {
    key := fmt.Sprintf("cache:user:id:%d", id)
    
    // 1. 先查 Redis
    var user User
    err := m.QueryRowCtx(ctx, &user, key, func(ctx, conn, v) error {
        // 2. Redis 未命中，查 MySQL
        return conn.QueryRowCtx(ctx, v, 
            "SELECT * FROM user WHERE id = ?", id)
    })
    
    return &user, err
}

// 更新时自动清除缓存
func (m *defaultUserModel) Update(ctx context.Context, data *User) error {
    key := fmt.Sprintf("cache:user:id:%d", data.Id)
    
    _, err := m.ExecCtx(ctx, func(ctx, conn) (sql.Result, error) {
        return conn.ExecCtx(ctx, "UPDATE user SET ... WHERE id = ?", ...)
    }, key)  // 自动删除缓存
    
    return err
}
```

---

### 5.3 唯一性检查

**更新手机号时检查**:
```go
func UpdateProfile(userId int64, phone string) error {
    // 检查手机号是否被其他用户使用
    var existUser User
    err := db.Where("phone = ? AND id != ?", phone, userId).First(&existUser).Error
    
    if err == nil {
        return errors.New("手机号已被使用")
    }
    
    // 更新
    return db.Model(&User{}).Where("id = ?", userId).Update("phone", phone).Error
}
```

---

## 六、性能优化策略

### 6.1 缓存优化

**1. Model 级缓存**（已实现）:
- go-zero 自动缓存
- 缓存时间：1 小时

**2. 搜索结果缓存**（可选）:
```go
func SearchUser(keyword string) ([]*UserInfo, error) {
    cacheKey := fmt.Sprintf("search:user:%s", keyword)
    
    // 先查缓存
    if cache.Exists(cacheKey) {
        return cache.Get(cacheKey), nil
    }
    
    // 查数据库
    users := db.Where("username = ? OR phone = ? OR email = ?", 
                      keyword, keyword, keyword).Find(&User{})
    
    // 写入缓存（5分钟）
    cache.Set(cacheKey, users, 5*time.Minute)
    
    return users, nil
}
```

---

### 6.2 数据库优化

**1. 索引优化**（已实现）:
```sql
UNIQUE KEY `uk_username` (`username`),
UNIQUE KEY `uk_phone` (`phone`),
UNIQUE KEY `uk_email` (`email`)
```

**2. 查询优化**:
```sql
-- ✅ 使用唯一索引
SELECT * FROM user WHERE username = 'skylm808';  -- 命中索引

-- ❌ 避免模糊查询（搜索除外）
SELECT * FROM user WHERE username LIKE '%skylm%';  -- 全表扫描
```

---

## 七、常见问题

### Q1: User 服务和 Auth 服务的区别？

**A**: 

**Auth 服务**：
- 用户注册、登录
- JWT Token 颁发和刷新
- 密码找回、修改密码

**User 服务**：
- 用户资料查询和更新
- 用户搜索

**为什么分开**？
- ✅ 职责分离：认证和资料管理是不同关注点
- ✅ 安全性：密码相关操作独立管理
- ✅ 扩展性：可以独立扩展

---

### Q2: 为什么不能修改用户名？

**A**: 
- `username` 是用户的唯一标识
- 用户名修改会导致：
  - 好友关系需要同步更新
  - 历史消息发送者需要更新
  - 群成员记录需要更新
- 允许修改 `nickname`（昵称）来实现"改名"需求

---

### Q3: 如何实现用户封号？

**A**: 
```go
// 封号
func BanUser(userId int64) error {
    return db.Model(&User{}).
        Where("id = ?", userId).
        Update("status", 0).Error
}

// 解封
func UnbanUser(userId int64) error {
    return db.Model(&User{}).
        Where("id = ?", userId).
        Update("status", 1).Error
}

// 登录时检查
func Login(username, password string) (*User, error) {
    user := getUserByUsername(username)
    
    if user.Status == 0 {
        return nil, errors.New("用户已被禁用")
    }
    
    // 验证密码...
}
```

---

## 八、总结

User 微服务采用**分层架构**：

1. **API 层**：HTTP 接口（资料、搜索）
2. **RPC 层**：内部服务调用
3. **Model 层**：数据访问（带缓存）

**核心特点**：
- 📊 与 Auth 服务职责分离
- 🔐 隐私保护（脱敏、权限控制）
- 🚀 Model 缓存提升性能
- ✅ 唯一性约束保证数据一致性
- 🔍 支持精确搜索和模糊搜索
- 🎯 简单高效，易于维护

---

**文档作者**: Skylm  
**最后更新**: 2026-01-13  
**相关文档**: [User API 文档](./api/USER_API文档.md)
